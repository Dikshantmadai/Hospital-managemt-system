...
class Admin:
    ...
    def login(self) :
        ...
        print("-----Welcome to Hospital system, please log in first-----")
    ...
    def get_doctor_details(self) :
        ...
        docto_fname = input("Please, enter the first name of the doctor: ")
        docto_sname = input("Please, enter the surname of the doctor: ")
        docto_spec = input("Please, enter the speciality of the doctor: ")

        return docto_fname,docto_sname,docto_spec
    ...
    def doctor_management(self, doctors):
        ...
        if op == '1':
            ...
            docto_fname, docto_sname, docto_spec = self.get_doctor_details()
            # check if the name is already registered
            name_exists = False
            print(docto_fname)
            print(docto_sname)
            for ab in doctors:
                if(docto_fname==ab.get_first_name() and docto_sname==ab.get_surname()):
                     print("Name already exists.")
                     break
            else:
                
                doctors.append(Doctor(docto_fname,docto_sname,docto_spec))
        ...
        # Delete
            ...
            index = int(input('Enter the ID of the doctor you want to remove: '))-1
            doctor_index=self.find_index(index,doctors)
            if(doctor_index!=False):
                doctors.pop(index)
                print("The doctor has been removedd")
                self.view(doctors)
    ...
    def discharge(self, patients, discharge_patients):
        ...
        y_no = input("Want to discharge this patient? (Y/N): ")
        if(y_no == 'y' or 'Y'):
             print("-----Patient' Discharge-----")
             pd_id = int(input("Please Enter the patient ID: "))-1
            #  yes_no = input("Do you want to discharge a patient(Y/N): ")
             if pd_id  in range(len(patients)):
                discharge_patients.append(patients.pop(pd_id))
                self.view_discharge(discharged_patients)
                self.view_patient(patients)
             else:
                print("Patient ID not valid.")
        else:
            print(" Fail to discharge .")
    ...
    def update_details(self):
        ...
        if op == 1:
            self.admin_details()
            username1 = input('Please, enter the new username: ')
            # validate the username
            username2 = input('Please, enter the new username again: ')
            if(username1 == username2):
                print("The username is a match.")
